...
/datum/build_mode/build/OnClick(atom/target, list/parameters)
	...
	if (parameters["right"])
		...
		if (ismob(target))
			...
			if (M.ckey && !QDELETED(M))
				...
				if (alert_result != "Yes, Delete" || QDELETED(M))
					return
			to_chat(M, SPAN_WARNING(FONT_LARGE("OOC: You have been deleted by an admin using build mode. If this seems to be in error, please adminhelp and let them know.")))
			M.ghostize()
		qdel(target)
		...
		if (!location)
			...
			instance.set_dir(host.direction)

/datum/build_mode/build/CanUseTopic(mob/user)
	if (!(check_rights(R_ADMIN, 0, (user)) != 0))
		return STATUS_CLOSE
	return ..()
