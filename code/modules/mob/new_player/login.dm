...
/mob/new_player/proc/whitelist_check()
	...
	if(dbcon.IsConnected())
		...
		var/DBQuery/find_ticket = dbcon.NewQuery(
			"SELECT ckey FROM [sqldbutil].ckey_whitelist WHERE ckey='[dbckey]' AND is_valid=true AND port=[world.port] AND date_start<=NOW() AND (NOW()<date_end OR date_end IS NULL)"
		)
